...
export default function ProductsPage() {
  ...
  const { data: all } = useQuery({ queryKey: ["products"], queryFn: () => repositories.products.getAll({ pageSize: 500 }) });
  ...
  return (
    <div>
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="all"><DataTable columns={columns} data={products} onRowClick={(r) => router.push(`/admin/products/${r.id}`)} /></TabsContent>
        {(["PENDING", "APPROVED", "REJECTED"] as const).map((s) => (
          <TabsContent key={s} value={s}><DataTable columns={columns} data={filterByStatus(s)} /></TabsContent>
        ))}
      </Tabs>
    </div>
  );
